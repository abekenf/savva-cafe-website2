...
- **Год в копирайте.** Таск 05 берёт его из `page.date.getFullYear()`, потому что
  Eleventy не даёт глобала с датой сборки, а `.eleventy.js` был вне его зоны. Это значит,
  что год следует за датой файла страницы, а не за календарём. Заведи в `.eleventy.js`
  глобальные данные с годом сборки и переключи подвал на них.
