...
**`data-to-top` в контракте нет — и не должно быть.** Кнопку «наверх» `app.js` создаёт
сам: хост-элемента в разметке не существует, потому что ни одна секция им не владеет.
Это единственная из восьми функций, которая порождает свой элемент, а не находит его;
в самом `app.js` это сказано комментарием. Искать `data-to-top` в шаблонах не надо.
