...
- `.eleventy.js` — шорткод `{% image src, alt, sizes="100vw", eager=false, classes="" %}`, пустой `alt` роняет сборку; `build.preloadFont` — путь к шрифту первого экрана, единый источник для `base.njk` и `main.css`.
...
- `src/js/app.js` — восемь функций поведения в `init()`, контракт — `data-*` атрибуты.
...
- Новый CSS-компонент заводится в `main.css` только если среди существующих (`.container-page`, `.section`, `.h-display`, `.eyebrow`, `.btn-primary`, `.btn-ghost`, `.card`, `.skip-link`) правда нет подходящего.
